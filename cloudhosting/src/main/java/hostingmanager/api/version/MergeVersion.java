...
public class MergeVersion {
    public static void main(String[] args) {
        try {
            ...
            mergeVersion(DOMAIN, PRODUCT_ID, CLIENT_ID, token);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public static JSONObject mergeVersion(String domain, String productId, String clientId, String token) {
        HttpPost post = new HttpPost(domain + "/api/cloudhosting/web/v1/version/merge");
        ...
        try (CloseableHttpResponse response = HttpClients.createDefault().execute(post)) {
            ...
            if (statusCode == HttpStatus.SC_OK) {
                if (StringUtils.equals(resultObject.getString("code"), "0")) {
                    System.out.println("merge version successfully");
                    return resultObject;
                }
            }
            System.out.println("merge version failed");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
