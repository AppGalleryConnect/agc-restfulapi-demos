...
public class ModifyVersionFileList {
    public static void main(String[] args) {
        try {
            ...
            modifyVersionFileList(DOMAIN, PRODUCT_ID, CLIENT_ID, token);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public static JSONObject modifyVersionFileList(String domain, String productId, String clientId, String token) {
        HttpPost post = new HttpPost(domain + "/api/cloudhosting/web/v1/version/modifyFilelist");
        ...
        try (CloseableHttpResponse response = HttpClients.createDefault().execute(post)) {
            ...
            if (statusCode == HttpStatus.SC_OK) {
                ...
                if (StringUtils.equals(resultObject.getString("code"), "0")) {
                    System.out.println("modify versionFile successfully");
                    return resultObject;
                }
            }
            System.out.println("modify versionFile failed");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
