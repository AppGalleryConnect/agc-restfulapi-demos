...
public class CreateVersion {
    public static void main(String[] args) {
        try {
            ...
            // create version
            createVersion(DOMAIN, PRODUCT_ID, CLIENT_ID, token);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public static JSONObject createVersion(String domain, String productId, String clientId, String token) {
        HttpPost post = new HttpPost(domain + "/api/cloudhosting/web/v1/version/create");
        ...
        requestObject.put("baseVersion", "5M4tGnMa******ZWJwew");
        ...
        try (CloseableHttpResponse response = HttpClients.createDefault().execute(post)) {
            ...
            if (statusCode == HttpStatus.SC_OK) {
                if (StringUtils.equals(resultObject.getString("code"), "0")) {
                    System.out.println("create version successfully");
                    return resultObject;
                }
            }
            System.out.println("create version failed");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
