...
public class CreateSite {
    public static void main(String[] args) {
        try {
            ...
            createSite(DOMAIN, PRODUCT_ID, CLIENT_ID, token);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public static JSONObject createSite(String domain, String productId, String clientId, String token) {
        HttpPost post = new HttpPost(domain + "/api/cloudhosting/web/v1/site/create");
        ...
        requestObject.put("siteName", "hosting****-test");
        requestObject.put("description", "1.0.0");
        ...
        try (CloseableHttpResponse response = HttpClients.createDefault().execute(post)) {
            ...
            if (statusCode == HttpStatus.SC_OK) {
                if (resultObject.containsKey("site")
                    && StringUtils.isNotBlank((resultObject.getJSONObject("site")).getString("siteId"))) {
                    System.out.println("create site successfully");
                    return resultObject;
                }
            }
            System.out.println("create site failed");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
