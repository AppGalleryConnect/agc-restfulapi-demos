...
public class ModifySite {
    public static void main(String[] args) {
        try {
            ...
            modifySite(DOMAIN, PRODUCT_ID, CLIENT_ID, token);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public static void modifySite(String domain, String productId, String clientId, String token) {
        HttpPost post = new HttpPost(domain + "/api/cloudhosting/web/v1/site/modify");
        ...
        requestObject.put("siteId", "otJNgf5z*****7Kmvkg");
        // cleanSuffix=1 https://fly.huawei.com/demo => https://fly.huawei.com/demo.html
        requestObject.put("cleanSuffix", 1);
        requestObject.put("description", "1.0.1");

        StringEntity entity = new StringEntity(requestObject.toString(), Charset.forName("UTF-8"));
        ...
        try (CloseableHttpResponse response = HttpClients.createDefault().execute(post)) {
            ...
            if (statusCode == HttpStatus.SC_OK) {
                if (StringUtils.equals(resultObject.getString("code"), "0")) {
                    System.out.println("modify site successfully");
                    return;
                }
                System.out.println("modify site failed");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
