...
public class CreateSiteEnvironment {
    public static void main(String[] args) {
        try {
            ...
            createSiteEnvironment(DOMAIN, PRODUCT_ID, CLIENT_ID, token);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public static JSONObject createSiteEnvironment(String domain, String productId, String clientId, String token) {
        HttpPost post = new HttpPost(domain + "/api/cloudhosting/web/v1/site/environment/create");
        ...
        requestObject.put("siteId", "otJNg*******kg");
        ...
        try (CloseableHttpResponse response = HttpClients.createDefault().execute(post)) {
            ...
            if (statusCode == HttpStatus.SC_OK) {
                if (StringUtils.equals(resultObject.getString("code"), "0")) {
                    System.out.println("create environment successfully");
                    return resultObject;
                }
            }
            System.out.println("create environment failed");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
